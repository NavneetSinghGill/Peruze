...
class ExclusivityController {
  ...
  private var operationsWithThisCategory = [Operation]()
  private func noqueue_addOperation(operation: Operation, category: String) {
    operationsWithThisCategory = operations[category] ?? []
  ...
  private func noqueue_removeOperation(operation: Operation, category: String) {
    ...
    var index: Int?
    for i in 0..<operationsWithThisCategory.count {
      if operation == operationsWithThisCategory[i] {
        index = i
        break
      }
    }
